...
## Method Utama
- `public int attack(Character target)`: Melancarkan serangan fisik naga Azhrax dengan kalkulasi damage dasar `calculateBaseAttack() + 5` (sekitar 20 damage).
- `public int useSkill(Character target)`: Mengembuskan skill legendaris `DRAGON BREATH` berdamage api masif `calculateBaseAttack() + 12` (sekitar 27 damage).
